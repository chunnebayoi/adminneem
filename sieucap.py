...
st.markdown('<div class="main-title">🎲 Phân Tích Cầu SUNWIN 🎲</div>', unsafe_allow_html=True)
    ...
    with st.container():
        ...
        if st.button("Tạo tài khoản"):
            if new_username in db['users']:
                ...
                st.info("📌 Vui lòng liên hệ admin để được cấp key kích hoạt.")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        with st.container():
            ...
            if st.button("Cấp key"):
                if admin_user in db['users']:
                    if admin_key in db['used_keys']:
                        st.error("❌ Key này đã được sử dụng, chọn key khác.")
                    else:
                        db['users'][admin_user]['active_key'] = admin_key
